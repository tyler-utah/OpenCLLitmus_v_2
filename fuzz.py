...
prestress_iterations = 100
prestress_pattern = [0, 1, 2, 3]

x_y_stride = range(33)[1:]
...
for j in range(int(iterations)):
    ...
    barrier_val = np.random.choice(barrier_switch)
    barrier_tag = " -D BARRIER=" + str(barrier_val)

    shuffle_val = np.random.choice(shuffle_switch)
    shuffle_tag = " -D ID_SHUFFLE=" + str(shuffle_val)

    stress_val = np.random.choice(stress_switch)
    ...
    flag = barrier_tag + shuffle_tag + stress_tag + stress_iterations_tag + stress_pattern_tag + prestress_tag + prestress_iterations_tag + prestress_pattern_tag + x_y_stride_tag
